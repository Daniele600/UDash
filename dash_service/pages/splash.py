...
from dash_service.models import Splashpage, Project, Page
...
def layout(lang="en", **query_params):
    ...
    if project_slug is None or page_slug is None:
        return render_no_splashpage_cfg_found(project_slug, page_slug)

    splash = Splashpage.query.join(Project).filter(and_(Page.slug == page_slug, Project.slug == project_slug)).first()

    config = splash.content

    return render_page_template(config, lang, query_params)


def render_no_splashpage_cfg_found(prj, page):
    if prj is None:
...
def render_page_template(
    ...
    **kwargs,
    ...
    if "main_title" in page_config:
        elem_main_title = HeadingAIO(page_config["main_title"], aio_id="splash_page_head")
